...
wandb.login(key="7097851f16b1dab3b892133d7afef252222d9172")
wandb.init(project="Fake_News_Article")
...
# Gộp dữ liệu từ các tập dữ liệu lại với nhau.
...
# Chức năng Tokenizing của BERT chuyển văn bản về đơn vị nhỏ hơn: Từ hoặc ký tự (còn gọi là token)
# Đưa văn bản về chuẩn chỉ lấy tối đa 255 tokens đầu tiên từ văn bản.
# Mục đích chung là để chuẩn hoá dữ liệu, chuẩn hoá độ dài dữ liệu. Chuyển các token về dạng ID, mỗi ID sẽ có 1 ID là số nguyên duy nhất.
...
# Chuyển thành tensors(Để Pytorch xử lý, vì BERT được xây dựng trên PyTorch)
...
class BertBinaryClassifier(nn.Module):
    ...
    def forward(self, tokens, masks=None):
        ...
        pooled_output = outputs.pooler_output  # Đây là tensor đầu ra từ BERT

        # Kiểm tra xem pooled_output có phải là tensor không
        assert isinstance(pooled_output, torch.Tensor), "pooled_output must be a tensor"
